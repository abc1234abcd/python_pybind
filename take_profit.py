...
class TakeProfit(MarketDataStreamer):
    __slots__ = ['api_client', 'msg_parser', 'rsi_calculator', 'price_buffer', 'rsi_value_buffer', 'volume_buffer', 'ob_cache', 'position', '_buy_order', '_sell_order', 'last_latency', 'filled_entry_price','filled_qty']
    ...
    async def _execute_strategy(self, rsi_value, file):
        ...
        oversold = self.oversold_threshold - (volatility * 100)
        overbought = self.overbought_threshold + (volatility * 100)
        ...
        self.rsi_value_buffer[0] = self.rsi_value_buffer[-1]
        self.rsi_value_buffer[-1] = rsi_value
        prev_rsi = self.rsi_value_buffer[0]
        ...
        if self.position == None:
            if (momentum > 1 and 
                ...
                if target_exit_price - self.filled_entry_price > self.filled_entry_price*0.001 and momentum < 0:
                ...
                elif target_exit_price - self.filled_entry_price < -(self.filled_entry_price*self.stop_loss_pct):
                    ...
                    try:
                        ...
                        if sell_order_status['status'] in ['FILLED', 'PARTIALLY_FILLED']:
                            ...
                            filled_sell_price = float(sell_order_status['cummulativeQuoteQty'])/filled_sell_qty
                            if filled_sell_qty == self.filled_qty:
                                print(f"stop loss order exec: loss: {filled_sell_price*self.filled_qty - 10.0}, entry: {self.filled_entry_price}, target entry: {target_entry_price}, exit: {filled_sell_price}, target exit: {target_exit_price}")
                        self.position = None
                        self.filled_entry_price = 0.0
                        self.filled_qty = 0.0
                    except Exception as e:
